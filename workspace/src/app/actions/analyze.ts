...
export async function getFastAnalysis(url: string): Promise<Partial<AnalysisResult> | { error: string }> {
  try {
    ...
    const scrapingBeeApiKey = process.env.SCRAPINGBEE_API_KEY || 'YOUR_SCRAPINGBEE_API_KEY';
    const scrapingUrl = `https://app.scrapingbee.com/api/v1/?api_key=${scrapingBeeApiKey}&url=${encodeURIComponent(url)}`;
    ...
    if (pageHtmlRes.status === 'rejected' || (pageHtmlRes.status === 'fulfilled' && !pageHtmlRes.value.ok)) {
        let errorMessage = 'Could not fetch the main page of the website. It might be down, blocking requests, or it might be a Single Page Application that requires JavaScript to render.';
        if(pageHtmlRes.status === 'fulfilled' && pageHtmlRes.value.status === 403) {
            errorMessage = 'The request was forbidden. The website may be blocking automated access. We are working on a solution for this.';
        }
        if(pageHtmlRes.status === 'fulfilled' && pageHtmlRes.value.status === 404) {
